...
appWorker.post('/collect', async (req, res) => {
    try {
        ...
        const { collectors } = await Promise.resolve().then(() => __importStar(require('./services/collector/registry')));
        const { saveCollectedJobs } = await Promise.resolve().then(() => __importStar(require('./services/collector/save')));
        const { dedupeJobs } = await Promise.resolve().then(() => __importStar(require('./services/collector/normalize')));
        ...
        if (missionFilter && missionFilter.activeMissions && missionFilter.activeMissions.length > 0) {
            mergedJobs = mergedJobs.filter((job) => missionFilter.activeMissions.some((mission) => {
...
appWorker.post('/apply', async (req, res) => {
    try {
        ...
        const { AutomationService } = await Promise.resolve().then(() => __importStar(require('./services/apply/automation-service')));
