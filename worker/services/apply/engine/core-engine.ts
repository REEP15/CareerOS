  ...
  async run(
    ...
    try {
      ...
      const decision = await hooks.requestConfirmation({
        ...
        reason: "final_submit",
        question:
          "The application is filled and ready. Do you authorize submitting it now?",
      });
      ...
      if (!submit) {
        hooks.log(
          "warn",
          "Submit authorized but no submit control was found; leaving staged for manual submit.",
        );
        return this.finalize(result, "completed_manual");
      }
