...
export interface ResolverInputs {
  ...
  /** Absolute local paths, pre-materialized by the FileProvider. */
  resumePath: string;
  coverLetterPath?: string;
}

/** Normalized profile the answer resolver reads from. Never mutated here. */
export interface UserProfile {
  ...
  workAuthorization?: {
    /** e.g. "US Citizen", "H-1B", "Requires sponsorship". Free-form, from onboarding. */
    status?: string;
  ...
  /**
   * Answers the user has explicitly saved for reuse (e.g. EEO responses,
   * "how did you hear about us"). Keyed by a stable question key.
   */
...
export interface ResumeVersion {
  id: string;
  label: string;
  /** Absolute path on the worker's filesystem OR a signed URL to download. */
  fileRef: string;
  fileName: string;
  contentType: string;
}
...
export interface CoverLetterVersion {
  ...
  fileName?: string;
  contentType?: string;
  /** Plain-text body, used for sites that accept a textarea instead of a file. */
  bodyText?: string;
}

/** Yes/no choice controls: pick the option matching a boolean intent. */
function pickBooleanOption(
...
/**
 * Map CareerOS ResumeProfile to UserProfile interface
 */
export function mapResumeProfileToUserProfile(resume: ResumeProfile, userId: string): UserProfile {
  ...
  return {
    ...
    address: {
      ...
      city: undefined, // Would need parsing from location
      state: undefined,
      country: undefined,
    ...
    links: {
      ...
      website: undefined, // ResumeProfile doesn't have website field
    },
    workAuthorization: {
      status: undefined, // Would need from onboarding
    ...
    customAnswers: {}, // Would need from user settings
  };
}

/**
 * Attempt to resolve a single field. Returns `null` when the answer cannot be
 * derived from known data — the engine treats that as "ask the user".
 */
export function resolveAnswer(
  ...
  // Sensitive EEO questions are never auto-answered, even if a value exists.
  ...
  // Cap answer confidence by how sure we are the field means what we think.
  ...
  switch (field.semantic) {
    ...
    default: {
      // Try a saved custom answer keyed by a normalized label.
